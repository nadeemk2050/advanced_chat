...
Future<void> _firebaseMessagingBackgroundHandler(RemoteMessage message) async {
  ...
  if (data['type'] == 'message') {
    ...
    if (chatRoomId != null && messageId != null) {
      try {
        // Stable settings for Web to avoid Assertion Errors
...
void main() async {
  ...
  if (kIsWeb) {
    // Disable persistence on Web to prevent ca9/b815 WebChannel crash.
    // This must be set BEFORE any Firestore listener is opened.
    FirebaseFirestore.instance.settings = const Settings(
      persistenceEnabled: false,
    );
  }
