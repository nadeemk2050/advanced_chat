...
class _GroupChatScreenState extends State<GroupChatScreen> {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: GestureDetector(
          onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => GroupDetailsScreen(group: widget.group, members: _membersCache.values.toList()))),
          child: Row(
            children: [
              CircleAvatar(
                radius: 18,
                backgroundColor: ChatTheme.primary.withOpacity(0.1),
                backgroundImage: (widget.group.groupPhotoUrl != null && widget.group.groupPhotoUrl!.isNotEmpty)
                    ? NetworkImage(widget.group.groupPhotoUrl!)
                    : null,
                child: (widget.group.groupPhotoUrl == null || widget.group.groupPhotoUrl!.isEmpty)
                    ? const Icon(Icons.group_rounded, size: 18, color: ChatTheme.primary)
                    : null,
              ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(widget.group.name, style: GoogleFonts.montserrat(fontWeight: FontWeight.bold, fontSize: 16)),
                    Text('${widget.group.members.length} agents', style: const TextStyle(fontSize: 10, color: Colors.grey)),
                  ],
                ),
              ),
            ],
        ...
        actions: [
          IconButton(
            ...
            tooltip: 'AI Extract Tasks',
            onPressed: _showAISuggestions,
          ),
          IconButton(
            icon: const Icon(Icons.info_outline_rounded),
            onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => GroupDetailsScreen(group: widget.group, members: _membersCache.values.toList()))),
          ),
        ],
      ),
      body: Column(
  ...
  void _showAISuggestions() async {
    ...
    showDialog(
      context: context,
      barrierDismissible: false,
      builder: (ctx) => const Center(child: SpinKitPulse(color: ChatTheme.primary)),
    );

    try {
      // Fetch last 20 messages from the messages sub-collection
      ...
      final messages = messagesSnap.docs.map((doc) => MessageModel.fromMap({
        ...(doc.data() as Map<String, dynamic>),
        'messageId': doc.id,
      })).toList();
      
      final suggestions = await _aiService.suggestTasksFromChat(messages);
      
      if (mounted) Navigator.pop(context); // Close loading

      if (suggestions.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('No tasks found in recent chat.')));
        return;
      }
  ...
  void _showApiKeyDialog() {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        backgroundColor: ChatTheme.surface,
        title: Text('SET GEMINI API KEY', style: GoogleFonts.montserrat(fontWeight: FontWeight.w900, fontSize: 14, color: ChatTheme.primary)),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            const Text('To extract tasks from chat, you need a Gemini API Key. (Get one at aistudio.google.com)', style: TextStyle(fontSize: 10, color: Colors.grey)),
      ...
      builder: (ctx) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text('AI TASK SUGGESTIONS', style: GoogleFonts.montserrat(fontWeight: FontWeight.w900, fontSize: 16, color: ChatTheme.primary)),
            const Text('Extracted from recent discussions', style: TextStyle(fontSize: 10, color: Colors.grey)),
            const SizedBox(height: 16),
            Flexible(
              child: ListView.builder(
                ...
                itemBuilder: (ctx, i) => Card(
                  ...
                  child: ListTile(
                    leading: const Icon(Icons.add_task_rounded, color: ChatTheme.primary, size: 20),
                    title: Text(suggestions[i], style: const TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.bold)),
                    trailing: const Icon(Icons.add_circle_outline_rounded, color: Colors.grey, size: 18),
                    onTap: () async {
                      final missionId = widget.group.missionId;
                      if (missionId != null) {
                        await _missionService.addTask(missionId, suggestions[i], _membersCache[_currentUid]?.name ?? 'User');
                        if (mounted) {
                          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                            content: Text('MISSION UPDATED: ${suggestions[i]}'),
                            backgroundColor: Colors.green.withOpacity(0.8),
                          ));
                        }
                      } else {
                        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Error: No linked mission found.')));
                      }
                    },
                  ),
                ),
