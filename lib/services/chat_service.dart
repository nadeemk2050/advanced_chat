...
class ChatService {
  ...
  /// Returns a stream of accepted peer UIDs using two collection queries merged
  /// by rxdart — replaces the previous pattern of one listener per list item.
  Stream<Set<String>> getAcceptedPeerIds() {
    final myUid = _auth.currentUser?.uid;
    if (myUid == null) return Stream.value({});
    final asSender = _firestore
        .collection('connections')
        .where('senderId', isEqualTo: myUid)
    ...
    final asReceiver = _firestore
        ...
        .map((s) => s.docs
            ...
            .toSet());
    return Rx.combineLatest2<Set<String>, Set<String>, Set<String>>(
        asSender, asReceiver, (a, b) => {...a, ...b});
  }
}
